...
import { UserCheck, ShieldCheck, Briefcase, ChevronDown, ArrowLeft, User, FileText, Award, Bookmark, Sparkles } from 'lucide-react';

interface CandidateNavbarProps {
  onNavigate: (tab: string) => void;
}

export const CandidateNavbar: React.FC<CandidateNavbarProps> = ({ onNavigate }) => {
  const { role, setRole } = useApp();
  const [roleDropdownOpen, setRoleDropdownOpen] = useState(false);

  return (
    <header className="sticky top-0 z-50 bg-white border-b border-sky-200 shadow-sm">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs font-bold text-slate-800">Applicant Workspace</span>
            </div>
          </div>
          ...
          <nav className="hidden md:flex items-center space-x-2">
            <button
              ...
              className="px-3.5 py-2 rounded-xl text-xs font-bold text-sky-700 bg-sky-50 border border-sky-200 flex items-center gap-1.5"
            >
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-xl text-xs font-bold text-slate-700 hover:text-sky-600 hover:bg-sky-50 flex items-center gap-1.5"
            >
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-sky-50 border border-sky-300 text-sky-800 hover:bg-sky-100 transition-all shadow-sm"
              >
              ...
              {roleDropdownOpen && (
                <div className="absolute right-0 mt-2 w-60 bg-white border border-slate-200 rounded-2xl shadow-xl p-2 z-50 animate-in fade-in">
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs font-bold rounded-xl bg-sky-50 text-sky-700 text-left"
                  >
